"""Command-line interface for testing Laya locally.
    ...
    laya "..." --predict --gate triage_gate.json         # apply a certified risk gate
    laya --gate triage_gate.json --report                # print what a saved gate certifies
...
def build_parser():
    ...
    parser.add_argument("--gate", metavar="PATH",
                        help="apply a saved ConformalGate (see laya.conformal); with --report, "
...
def main(argv=None):
    ...
    if args.gate:
        try:
            ...
            print("laya: could not load the gate at %s (%s)" % (args.gate, error), file=sys.stderr)
            return 2
        if args.report:
            print(gate.report())
            return 0
        if not args.predict:
            ...
            print("laya: --gate applies to answers, so it needs --predict "
                  "(or --report to describe the gate)", file=sys.stderr)
            return 2
    elif args.report:
        print("laya: --report needs --gate PATH", file=sys.stderr)
        return 2
