"""taut-eval: an independent, reproducible evaluation harness for Taut checkpoints.
...
party can point at a Taut checkpoint to get a per-language accuracy and ECE
...
Usage
-----
    python -m taut_eval --model thekarteek/taut --langs en,de,ro
    python -m taut_eval --model ./local-checkpoint --langs all --out report.json
    python -m taut_eval --model thekarteek/taut --subfolder multilingual --langs all
...
# `import taut` would fail. Allow both `python research/eval/taut_eval.py` and
# `python -m research.eval.taut_eval` to find the package.
...
def score_cases(agent, cases) -> List[Any]:
    ...
    """
    import torch
    from taut.common import QTYPES, build_sequence, collate_items, render_options
...
def ece(confidence, correct, bins: int = ECE_BINS) -> float:
    ...
    That is the boundary #39 settled in `taut.common.ece_score`,
...
def run_language(agent, lang: str, per_lang: int, n_opts: int, seed: int = SEED,
    ...
    import numpy as np
    from taut.common import QTYPES
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        prog="taut-eval",
        description="Per-language accuracy and calibration report for a Taut checkpoint.")
    parser.add_argument("--model", default="thekarteek/taut",
    ...
    started = time.time()
    agent = taut.load(args.model, device=args.device, subfolder=args.subfolder)
    agent.model.eval()
    payload: Dict[str, Any] = {
        "config": {
            ...
            "taut_version": getattr(taut, "__version__", "unknown"),
        },
        "report": {},
        "cases": [],
    }
