"""GPU fast path for taut: TileLang fused kernels + bf16 resident weights + CUDA graphs.

    agent = taut.load("thekarteek/taut", fast=True)      # or agent.accelerate()

Requires CUDA and `pip install taut[fast]` (tilelang).  Falls back to the stock forward otherwise.
"""
import sys
import torch
...
class FastTaut:
    ...
    def _encode_graphed(self, ids, lens, qtype):
        ...
        if g is None:
            ...
            if self.verbose:
                print(f"[fast_taut] captured CUDA graph for shape {key}", file=sys.stderr)
    ...
    def forward(self, input_ids, attention_mask, marker_pos, marker_mask, qtype, detach_encoder=False):
        ...
        h = h[:N, :L0].float()
        # ---- scorer / act head (tiny; identical to taut.common.DecisionModel.forward)
