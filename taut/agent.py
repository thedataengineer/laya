"""High-level inference runtime for taut System 1 decision models."""
import json
import os
import threading
import warnings
...
# The Hugging Face account Taut publishes its checkpoint mirrors under. Overridable so a
# fork, a private mirror, or the upstream originals can be used without editing code.
_MIRROR_ORG = os.environ.get("TAUT_MODEL_ORG", "thekarteek")


def _is_missing_repo(exc: Exception) -> bool:
    """Whether a hub exception means "no such repository" rather than a transport fault."""
    try:
        from huggingface_hub.errors import RepositoryNotFoundError
    except ImportError:  # huggingface_hub < 0.23 kept it in utils
        try:
            from huggingface_hub.utils import RepositoryNotFoundError
        except ImportError:
            RepositoryNotFoundError = ()  # type: ignore[assignment]
    if RepositoryNotFoundError and isinstance(exc, RepositoryNotFoundError):
        return True
    return "401" in str(exc) or "404" in str(exc) or "not found" in str(exc).lower()
...
def _fix_tokenizer_config(path: str):
    ...
    try:
        ...
        warnings.warn(
            "taut: could not patch %s (%s); the tokenizer may fail to load with this "
...
class Agent:
    ...
    def __init__(
        self,
        model_id_or_path: str = "thekarteek/taut",
        ...
        if not os.path.exists(model_dir):
            ...
            try:
                ...
                # Taut republishes upstream's Apache-2.0 weights under its own account.
                # A fresh clone whose owner has not mirrored them yet hits this first, and
                # the hub's own "repository not found" says nothing about how to fix it.
                if _is_missing_repo(e) and model_id_or_path.startswith(_MIRROR_ORG + "/"):
                    raise FileNotFoundError(
                        "Checkpoint %r is not published yet.\n"
                        "Taut mirrors upstream's Apache-2.0 Laya weights under %s/. Publish "
                        "them once with:\n\n"
                        "    python scripts/mirror_checkpoints.py\n\n"
                        "or point Taut at the upstream originals instead by setting\n"
                        "    TAUT_MODEL_ORG=convaiinnovations\n"
                        % (model_id_or_path, _MIRROR_ORG)
                    ) from e
                raise
        ...
        if not os.path.exists(cfg_path):
            raise FileNotFoundError(
                ...
                f"That file ships with the weights of a Taut checkpoint, so load one of those "
                f"(e.g. 'thekarteek/taut') or a directory your own training run wrote."
            )
        ...
        # That is a loss for the batch sizes Taut runs (a handful of questions per call) and can
        ...
        if rejected:
            warnings.warn(
                "taut: this checkpoint ships invalid temperatures or values outside [%g, %g]; "
        ...
        if fell_back_from is not None:
            print(
                "\n[taut] Warning: could not place the model on %s, so it is running on CPU.\n"
                "  Reason: %s\n"
    ...
    def accelerate(self, use_graphs: bool = True, strict: bool = False):
        ...
        if self.device.type != "cuda":
            if strict:
                raise RuntimeError("taut fast path needs a CUDA device")
            return False
        last = None
        for _attempt in range(2):  # tilelang's JIT cache has been seen to fail once, then succeed
            try:
                from .fast import FastTaut
                self._fast = FastTaut(self.model, max_len=self.cfg.get("max_len", 512), use_graphs=use_graphs)
                break
        ...
        if self._fast is None:
            if strict:
                raise last
            print("Warning: taut fast path unavailable (%s); using the stock forward." % last)
            return False
    ...
    def predict_batch(self, states: List[Union[str, dict, list]], questions: Dict[str, Dict[str, Any]],
        ...
        if not ids:
            return [{"model": "taut-rl-agent", "answers": {}, "usage": {"input_tokens": 0, "output_tokens": 0}}
                    for _ in states]
...
def load(model_id_or_path: str = "thekarteek/taut", device: Optional[str] = None,
    ...
    `subfolder` picks one checkpoint out of a repo that bundles several:
        ...
        taut.load("thekarteek/taut")                           # English (repo root)
        taut.load("thekarteek/taut", subfolder="multilingual")
        taut.load("thekarteek/taut", fast=True)                # TileLang GPU fast path
    """
