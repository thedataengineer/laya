"""Route a request to the Taut checkpoint best suited to it.
...
per model -- see the repository's benchmark notebook):
  ...
  english          thekarteek/taut                421M  ModernBERT-large, 512 tokens
  multilingual     thekarteek/taut-multilingual   322M  mmBERT-base, 1024 tokens, 100+ langs
  typed-decisions  thekarteek/taut-typed-decisions 421M  ModernBERT-large, 1024 tokens,
...
# The Hugging Face account the checkpoints are loaded from, and the repo-name stem under
# it. Taut mirrors upstream's Apache-2.0 weights rather than retraining them, so both are
# overridable: `TAUT_MODEL_ORG=convaiinnovations TAUT_MODEL_STEM=laya` loads the upstream
# originals unchanged, and a private mirror needs only the org. Read at import so a process
# sees one consistent pair.
MODEL_ORG = os.environ.get("TAUT_MODEL_ORG", "thekarteek")
MODEL_STEM = os.environ.get("TAUT_MODEL_STEM") or ("laya" if MODEL_ORG == "convaiinnovations"
                                                   else "taut")

# The hub repo bundles all three checkpoints; only the requested subfolder is downloaded.
BUNDLE_REPO = "%s/%s" % (MODEL_ORG, MODEL_STEM)
DEFAULT_MODELS = {
...
STANDALONE_MODELS = {
    "english": BUNDLE_REPO,
    "multilingual": "%s-multilingual" % BUNDLE_REPO,
    "typed-decisions": "%s-typed-decisions" % BUNDLE_REPO,
}
...
_ALIASES = {
    "en": "english", "taut": "english", "default": "english",
    "multi": "multilingual", "ml": "multilingual", "taut-multilingual": "multilingual",
    "typed": "typed-decisions", "typed_decisions": "typed-decisions",
    "taut-typed-decisions": "typed-decisions", "decisions": "typed-decisions",
}
...
class Router:
    """Lazily loads Taut checkpoints and sends each request to the right one.
    ...
    def attach(self, name: str, agent: Any):
        ...
        built `thekarteek/taut` can hand it to the router rather than pay for -- and hold
