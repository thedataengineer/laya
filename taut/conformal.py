"""Distribution-free risk control for Taut decisions.

Taut reports a calibrated probability. This module turns that probability into an
...
def _sketch(scores: np.ndarray) -> Dict[str, Any]:
    ...
    be resolved, and :mod:`taut.drift` accounts for that rather than pretending it is exact.
    """
...
# --------------------------------------------------------------------------------------
# Pulling scores out of Taut answers
# --------------------------------------------------------------------------------------
...
def _answer_distribution(answer: Mapping[str, Any]) -> Tuple[str, List[str], np.ndarray]:
    """(question type, option keys, probability vector) for any Taut answer."""
...
class ConformalGate:
    ...
    def report(self) -> str:
        ...
        lines = ["Taut conformal gate  alpha=%g  delta=%g  n=%s"
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "ConformalGate":
        ...
        if version is not None and int(version) > _SCHEMA_VERSION:
            raise ValueError("gate was written by a newer Taut (schema %s > %s); upgrade taut "
