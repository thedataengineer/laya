"""Device resolution: TAUT_DEVICE has the same meaning as in taut.serve.
...
(TAUT_DEVICE or torch auto-detection), not of the device a loaded checkpoint
...
_ENV_KEY = "TAUT_DEVICE"


def env_device() -> str | None:
    """TAUT_DEVICE verbatim (empty or unset = None), ready for Router(device=...)."""
...
def resolve_device(force: str | None = None) -> str:
    ...
    Priority:
      ...
      2. ``TAUT_DEVICE`` environment variable (the same value serve passes to torch)
...
def device_report() -> dict:
    """Structured device info for taut_status.
...
def router_agent(router: Any, name: str) -> Any | None:
    ...
    try:
        from taut.router import normalise_name
