...
export class Agent {
  ...
  constructor(opts: AgentOptions) {
    ...
    if (rejected.length > 0) {
      console.warn(
        `taut: this checkpoint ships invalid temperatures or values outside [${TEMP_MIN}, ${TEMP_MAX}]; ` +
  ...
  async systemOne(state: unknown, questions: Record<string, QuestionDef>): Promise<SystemOneResult> {
    ...
    return { model: "taut-rl-agent", answers, usage: { input_tokens: nTokens, output_tokens: 0 } };
  }
