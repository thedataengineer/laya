"""Export a Taut checkpoint to split ONNX (encoder.onnx + head.onnx).

Source of truth stays ``model.safetensors``; this script produces the two ONNX
files the ``taut-ts`` Node/Web providers load. Run once per checkpoint:
    python taut-ts/scripts/export_onnx.py --repo thekarteek/taut --out-dir ./model
    python taut-ts/scripts/export_onnx.py --repo thekarteek/taut --subfolder multilingual --out-dir ./model-ml
    python taut-ts/scripts/export_onnx.py --model-dir <local ckpt> --out-dir ./model
...
try:
    from taut.common import build_model
except ImportError:  # --help must work without torch installed
    build_model = None

DEFAULT_REPO = "thekarteek/taut"
...
def main(argv=None):
    ...
    if build_model is None:
        raise SystemExit("export needs torch + taut installed: pip install torch transformers onnx onnxruntime")
